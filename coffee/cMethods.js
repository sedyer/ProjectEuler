...
(function() {
  var comparator, findPythagoreanTripletWithGivenSum, getAllFactorPairs, getTriplet, getTripletsFromFactorPairs, getUniqueElements, getUniqueFactorPairs, getValidTriplets, isPrime, sortTuples, sumAllPrimesUnderValue;
  ...
  isPrime = function(num) {
    ...
    return (function() {
      results = [];
      for (var j = 2, ref = Math.ceil(Math.sqrt(num)); 2 <= ref ? j <= ref : j >= ref; 2 <= ref ? j++ : j--){ results.push(j); }
      return results;
  ...
  sumAllPrimesUnderValue = function(num) {
    var j, results;
    return (function() {
      results = [];
      for (var j = 2; 2 <= num ? j <= num : j >= num; 2 <= num ? j++ : j--){ results.push(j); }
      return results;
